...
def hinge (K : Real) (x : Real) : Real := max (x - K) 0
\end{lstlisting}

The option span is defined as the set of functions expressible as finite linear combinations:
\begin{lstlisting}[language=Haskell]
def OptionSpan (a b : Real) : Set (Real -> Real) :=
  { g | exists (alpha beta : Real) (N : Nat) (Ks : Fin N -> Real) (ws : Fin N -> Real),
      (forall i, Ks i in Set.Icc a b) /\
...
\begin{enumerate}
    ...
    \begin{lstlisting}[language=Haskell]
lemma hinge_nonneg (K x : Real) : 0 <= hinge K x := le_max_right _ _
    \end{lstlisting}

    \item \textbf{Zero region} (\texttt{hinge\_zero\_for\_x\_le\_K}): For $x \leq K$, $\phi_K(x) = 0$.
    \begin{lstlisting}[language=Haskell]
lemma hinge_zero_for_x_le_K (K x : Real) (h : x <= K) : hinge K x = 0 := by
  unfold hinge
  rw [max_eq_right]
  linarith
    \end{lstlisting}
    ...
    \begin{lstlisting}[language=Haskell]
lemma hinge_linear_for_x_ge_K (K x : Real) (h : K <= x) : hinge K x = x - K := by
  unfold hinge
  rw [max_eq_left]
  linarith
    \end{lstlisting}
    ...
    \begin{lstlisting}[language=Haskell]
theorem hinge_continuous (K : Real) : Continuous (hinge K) := by
  unfold hinge
  exact continuous_id.sub continuous_const |>.max continuous_const
    ...
    \begin{lstlisting}[language=Haskell]
theorem hinge_lipschitz (K : Real) : LipschitzWith 1 (hinge K) := by
  unfold hinge
  ...
  calc dist (max (x - K) 0) (max (y - K) 0)
    ...
    \begin{lstlisting}[language=Haskell]
lemma finite_sum_hinges_continuous (N : Nat) (Ks : Fin N -> Real) (ws : Fin N -> Real) :
  ...
  exact Continuous.mul continuous_const (hinge_continuous (Ks i))
    ...
    is continuous.
    \begin{lstlisting}[language=Haskell]
theorem option_portfolio_continuous (alpha beta : Real) (N : Nat) (Ks : Fin N -> Real) (ws : Fin N -> Real) :
  ...
  . exact finite_sum_hinges_continuous N Ks ws
    ...
    \begin{lstlisting}[language=Haskell]
theorem hinge_in_C_ab (a b K : Real) :
    exists f : C_ab a b, forall x : Set.Icc a b, f x = hinge K (x : Real) := by
  use <fun x => hinge K (x : Real),
...
\begin{lstlisting}[language=Haskell]
axiom UniversalApproximationTheoremReLU (a b : Real) (hab : a < b)
  (f : C_ab a b) (eps : Real) (heps : 0 < eps) :
  exists (N : Nat) (Ks : Fin N -> Real) (alpha beta : Real) (ws : Fin N -> Real),
    (forall i, Ks i in Set.Icc a b) /\
    (forall x : Set.Icc a b,
      |f x - (alpha + beta * (x : Real) +
        Finset.univ.sum (fun i => ws i * hinge (Ks i) (x : Real)))| < eps)
\end{lstlisting}
...
\begin{enumerate}
    \item \textbf{Type discipline}: Working in Lean forces precise type handling. The continuous functions on $[a,b]$ are represented as the type \texttt{C(Set.Icc a b, Real)}, which embeds the compactness of the domain into the type itself.
