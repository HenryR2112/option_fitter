...
We establish that the basis consisting of affine terms (constant and linear) together with call option payoffs is sufficient to uniformly approximate any continuous function on a compact interval. Beyond mere density, we derive quantitative convergence rates, characterize optimal approximations, and extend the theory to Sobolev spaces and functions with generalized derivatives.
...
\begin{enumerate}
    ...
    \item \textbf{Formal verification}: A Lean~4 formalization of core results, including continuity and Lipschitz properties of hinge functions.
...
def hinge (K : R) (x : R) : R := max (x - K) 0
\end{lstlisting}

The option span is defined as the set of functions expressible as finite linear combinations:
\begin{lstlisting}[language=Haskell]
def OptionSpan (a b : R) : Set (R -> R) :=
  { g | exists (alpha beta : R) (N : Nat)
        (Ks : Fin N -> R) (ws : Fin N -> R),
      (forall i, Ks i in Set.Icc a b) and
      forall x, g x = alpha + beta * x +
        (Finset.univ.sum fun i => ws i * hinge (Ks i) x) }
\end{lstlisting}

\subsection{Fully verified results}

The following results have complete machine-checked proofs:

\begin{enumerate}
    \item \textbf{Non-negativity}: \texttt{hinge\_nonneg}: $\phi_K(x) \geq 0$ for all $x$.
    ...
    \item \textbf{Continuity}: \texttt{hinge\_continuous}: The hinge function is continuous.
    \begin{lstlisting}[language=Haskell]
theorem hinge_continuous (K : R) : Continuous (hinge K) := by
  unfold hinge
  exact continuous_id.sub continuous_const |>.max continuous_const
    \end{lstlisting}

    \item \textbf{Lipschitz property}: \texttt{hinge\_lipschitz}: The hinge function is Lipschitz with constant $1$.
    \begin{lstlisting}[language=Haskell]
theorem hinge_lipschitz (K : R) : LipschitzWith 1 (hinge K) := by
  ...
  calc dist (max (x - K) 0) (max (y - K) 0)
    ...
    _ <= |x - K - (y - K)| := abs_max_sub_max_le_abs ...
    _ = |x - y| := by ring_nf
    _ = dist x y := (Real.dist_eq x y).symm
    \end{lstlisting}

    \item \textbf{Finite sums}: \texttt{finite\_sum\_hinges\_continuous}: Finite sums of weighted hinges are continuous.

    \item \textbf{Portfolio continuity}: \texttt{option\_portfolio\_continuous}: Any option portfolio $\alpha + \beta x + \sum_i w_i \phi_{K_i}(x)$ is continuous.

    \item \textbf{Boundedness}: \texttt{hinge\_bounded\_on\_interval}: The hinge function is bounded on $[a,b]$:
    \[|\phi_K(x)| \leq \max(b-a, |K-a|) + (b-a) \quad \text{for } x \in [a,b].\]
\end{enumerate}

\subsection{Theorem statements}

The main density theorem and integral representation are stated with their structure verified, though the full proofs use \texttt{sorry} placeholders:

\begin{lstlisting}[language=Haskell]
theorem density_of_call_spans_statement (a b : R) (hab : a < b)
    (f : C_ab a b) (eps : R) (h_eps : 0 < eps) :
    exists (N : Nat) (Ks : Fin N -> R) (alpha beta : R)
           (ws : Fin N -> R),
      (forall i, Ks i in Set.Icc a b) and
      (forall x : Set.Icc a b,
        |f x - (alpha + beta * x +
          (Finset.univ.sum fun i => ws i * hinge (Ks i) x))| < eps)
\end{lstlisting}
...
Result & Type & Status \\
\midrule
\texttt{hinge\_nonneg} & Lemma & Verified \\
\texttt{hinge\_zero\_for\_x\_le\_K} & Lemma & Verified \\
\texttt{hinge\_linear\_for\_x\_ge\_K} & Lemma & Verified \\
\texttt{hinge\_continuous} & Theorem & Verified \\
\texttt{hinge\_lipschitz} & Theorem & Verified \\
\texttt{finite\_sum\_hinges\_continuous} & Lemma & Verified \\
\texttt{option\_portfolio\_continuous} & Theorem & Verified \\
\texttt{constant\_in\_C\_ab} & Theorem & Verified \\
\texttt{linear\_in\_C\_ab} & Theorem & Verified \\
\texttt{hinge\_in\_C\_ab} & Theorem & Verified \\
\texttt{hinge\_bounded\_on\_interval} & Lemma & Verified \\
\texttt{density\_of\_call\_spans\_statement} & Theorem & Statement only \\
\texttt{integral\_representation\_C2\_statement} & Theorem & Statement only \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Formal verification}: Core results are machine-checked in Lean~4, providing rigorous foundations (Section~\ref{sec:lean}).
